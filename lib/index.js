              ...
              if (args[0]) {
                ...
                checkRunning(function(item, isRunning) {
                  if (item.command.match(regex)) {
                    var output = "Log file output for: "+item.command+"\n\n";
                    var filename = item.log_file;
                    fs.stat(filename, function(err, stats) {
                      if (err) {
                        sendError('missing log file for: '+item.command);
                      } else {
                        var start = stats.size > 1000 ? (stats.size - 1000) : 0;
                        var stream = fs.createReadStream(item.log_file, {start: start, end: stats.size});
                ...
                sendError('specify a regex to match a command on');
              }
              break;
            case 'reload':
  ...
  if (config.restartables && config.restartables.constructor === Array) {
      ...
      keepAliveInterval = setInterval(function() {
        checkRunning(function(item, isRunning) {
          ...
          if (pid && !isRunning && !item.ignore) {
            sendEmail("[restarter] command dies: "+item.command, "PID: "+pid);
    ...
    if (config.global_watch_file) {
      ...
      fs.watchFile(config.global_watch_file, function () {
        sendEmail("[restarter] global watch file touched, reloading restarter", "file: "+config.global_watch_file);
        reloadServer();
      });
    }
    config.restartables.forEach(function(item) {
      ...
      if (watchFile) {
        ...
        fs.watchFile(watchFile, function () {
          sendEmail("[restarter] watch file touched, restarting command: "+item.command, "no text");
